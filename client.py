...
def main():
    ...
    while run:
        ...
        screen.fill((0, 0, 0))
        if win:
            # print(pygame.time.get_ticks() - win_dialog_start_time)
            if pygame.time.get_ticks() - win_dialog_start_time > win_dialog_timer:
                win=False
                game = Game()
                stage = Stage(screen, game)
                page = Page()
            if stage.stage == page.game:
                ...
                stage.option_page()
            win_text = "Player {} wins!".format(winer)
            text_surface = font.render(win_text, True, (34,43, 166))
            text_rect = text_surface.get_rect(
